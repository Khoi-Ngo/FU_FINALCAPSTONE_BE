...
public class GitRepoService : IGitRepoService
{
    ...
    public async Task<object> ViewDataMetricOfGitUsernameAsync(string gitAccountUsername)
    {
        try
        {
            ...
            var graphQlQuery = @"query ($login: String!) {
                user(login: $login) {
                    login
                    name
                    bio
            ...
            {
                query = graphQlQuery,
                variables = new { login = gitAccountUsername }
            };

            GitHubUserData? userData = null;
            try
            {
                var content = new StringContent(JsonSerializer.Serialize(requestBody), Encoding.UTF8, "application/json");
                var graphQlResponse = await client.PostAsync("https://api.github.com/graphql", content);
                ...
                var jsonResponse = await graphQlResponse.Content.ReadAsStringAsync();
                var graphResponse = JsonSerializer.Deserialize<GraphQLUserResponse>(jsonResponse, options);
                if (graphResponse?.Data?.User == null)
                {
                    return null; // Return null if no user data
                }

                var graphUser = graphResponse.Data.User;
                userData = new GitHubUserData
                {
                    Login = graphUser.Login,
                    Name = graphUser.Name,
                    Bio = graphUser.Bio,
                    Company = graphUser.Company,
                    Location = graphUser.Location,
                    Email = graphUser.Email,
                    WebsiteUrl = graphUser.WebsiteUrl,
                    TwitterUsername = graphUser.TwitterUsername,
                    AvatarUrl = graphUser.AvatarUrl,
                    Url = graphUser.Url,
                    CreatedAt = graphUser.CreatedAt,
                    UpdatedAt = graphUser.UpdatedAt,
                    FollowersCount = graphUser.Followers?.TotalCount ?? 0,
                    FollowingCount = graphUser.Following?.TotalCount ?? 0,
                    GistsCount = graphUser.Gists?.TotalCount ?? 0,
                    RepositoriesCount = graphUser.Repositories?.TotalCount ?? 0,
                    RepositoriesContributedToCount = graphUser.RepositoriesContributedTo?.TotalCount ?? 0,
                    StarredRepositoriesCount = graphUser.StarredRepositories?.TotalCount ?? 0,
                    WatchingCount = graphUser.Watching?.TotalCount ?? 0,
                    IssuesCount = graphUser.Issues?.TotalCount ?? 0,
                    PullRequestsCount = graphUser.PullRequests?.TotalCount ?? 0,
                    TotalCommitContributions = graphUser.ContributionsCollection?.TotalCommitContributions ?? 0,
                    TotalIssueContributions = graphUser.ContributionsCollection?.TotalIssueContributions ?? 0,
                    TotalPullRequestContributions = graphUser.ContributionsCollection?.TotalPullRequestContributions ?? 0,
                    TotalPullRequestReviewContributions = graphUser.ContributionsCollection?.TotalPullRequestReviewContributions ?? 0,
                    TotalRepositoriesWithContributedCommits = graphUser.ContributionsCollection?.TotalRepositoriesWithContributedCommits ?? 0,
                    TotalContributions = graphUser.ContributionsCollection?.ContributionCalendar?.TotalContributions ?? 0,
                    ContributionsPerDayLastYear = new Dictionary<string, int>()
                };

                // Populate contributions per day
                var contributionsPerDay = new Dictionary<string, int>();
                if (graphUser.ContributionsCollection?.ContributionCalendar?.Weeks != null)
                {
                    foreach (var week in graphUser.ContributionsCollection.ContributionCalendar.Weeks)
                    {
                        if (week.ContributionDays != null)
                        {
                            foreach (var day in week.ContributionDays)
                            {
                                ...
                                {
                                    contributionsPerDay[day.Date] = day.ContributionCount;
                                }
                            }
                        }
                    }
                }
                userData.ContributionsPerDayLastYear = contributionsPerDay;
...
public class FocusedRepoData
{
    ...
    public int TotalCommitsLastYear { get; set; }
...
public class GitHubUserData
{
    ...
    public int GistsCount { get; set; }
    public int RepositoriesCount { get; set; }
    public int RepositoriesContributedToCount { get; set; }
    public int StarredRepositoriesCount { get; set; }
    ...
    public Dictionary<string, int> ContributionsPerDayLastYear { get; set; }
}

public class GraphQLUserResponse
{
    public GraphQLUserData Data { get; set; }
}
...
public class GraphQLUser
{
    ...
    public DateTimeOffset CreatedAt { get; set; }
    public DateTimeOffset UpdatedAt { get; set; }
    public TotalCountObject Followers { get; set; }
    public TotalCountObject Following { get; set; }
    public TotalCountObject Gists { get; set; }
    public TotalCountObject Repositories { get; set; }
    public TotalCountObject RepositoriesContributedTo { get; set; }
    public TotalCountObject StarredRepositories { get; set; }
    public TotalCountObject Watching { get; set; }
    public TotalCountObject Issues { get; set; }
    public TotalCountObject PullRequests { get; set; }
