...
{
    /// <inheritdoc />
    public partial class RefineLeaveTriggerCheck01 : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            // Recreate trigger with refined meeting check and availability logic
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    -- Check for ACTIVE BookedMeeting statuses: 1 (PENDING), 2 (CONFIRMED), 4 (COMPLETED), 5 (STUDENT_MISSED), 6 (ADVISOR_MISSED)
                    IF EXISTS (
                        SELECT 1
                        FROM BookedMeeting bm
                        INNER JOIN inserted AS new
                            ON bm.StaffProfileId = new.StaffProfileId
                            AND bm.Status IN (1, 2, 4, 5, 6)
                            AND bm.StartDateTime < new.EndDateTime
                            AND bm.EndDateTime > new.StartDateTime
                    )
                    BEGIN
                        THROW 50005, 'Cannot register leave due to existing active meetings. Cancel those meetings first.', 1;
                        ROLLBACK TRANSACTION;
                        RETURN;
                    END

                    -- BookingAvailability matching check (time range overlaps availability on same weekday)
                    IF NOT EXISTS (
                        SELECT 1
            ...
            migrationBuilder.Sql(@"
                EXEC sp_addextendedproperty 
                    ...
                    @value = N'Trigger to validate overlapping leave schedules, existing active meetings, and booking availability.', 
