...
namespace AISEA.ApiService.DAL.Repositories
{
    public class SubjectRepository : GenericRepository<Subject>
    {
        ...
        public async Task<Syllabus> GetSylaByIdWithAllRelatedAsync(long id)
        {
            return await _context.Syllabi
                .Where(s => s.Id == id)
                .Include(s => s.SyllabusAssessments)
                .Include(s => s.SyllabusLearningMaterials)
                .Include(s => s.SyllabusLearningOutcomes)
                .Include(s => s.SyllabusSessions)
                .FirstOrDefaultAsync();
        }
